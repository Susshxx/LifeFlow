...
function isValidEmergencyRequest(r: any): r is EmergencyRequest {
  return (
    r &&
    typeof r === 'object' &&
    r._id &&
...
export function DonorDashboard() {
    ...
    const fetchRecentActivity = async () => {
      ...
      try {
        ...
        if (historyRes.ok) {
          ...
          const activities = (Array.isArray(donations) ? donations : []).slice(0, 5).map((donation: any) => ({
            id: donation._id,
            type: 'donation',
            icon: DropletIcon,
            title: 'Blood Donation Completed',
    ...
    const fetchUpcomingCamps = async () => {
      ...
      try {
        ...
        if (campsRes.ok) {
          ...
          const upcoming = (Array.isArray(camps) ? camps : [])
            .filter((camp: any) =>
              camp &&
              camp._id &&
              (camp.status === 'approved' || camp.status === 'upcoming') &&
  ...
  const loadEmergencies = () => {
    try {
      ...
      if (cached) {
        ...
        const myUserId = storedUser?.id || storedUser?._id;
        const filtered = data
          .filter(isValidEmergencyRequest)
          .filter(r => {
            const requestUserId = typeof r.userId === 'object' ? (r.userId as any)._id : r.userId;
            return requestUserId !== myUserId;
          });
  ...
  useEffect(() => {
    ...
    fetchBloodRequestNotification();
    const interval = setInterval(fetchBloodRequestNotification, 30000);
    return () => clearInterval(interval);
  }, []);
