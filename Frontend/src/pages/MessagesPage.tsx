...
//       // Filter out connections where from or to is null/missing
//       const valid = data.filter(c => c && c.from && c.from._id && c.to && c.to._id);
...
//         (m) => m && m.sender && m.sender._id && m.sender._id !== myId && m.content && m.content.includes('BLOOD_REQUEST_NOTIFICATION')
//       );
//       if (!notificationMsg) {
...
//   if (!conn || !conn.from || !conn.to) return false;  // ADD THIS
