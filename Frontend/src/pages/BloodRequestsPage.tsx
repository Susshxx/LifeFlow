...
// import {
//   FilterIcon, MapIcon, ListIcon, AlertTriangleIcon,
//   LocateIcon, NavigationIcon, PhoneIcon, MailIcon, MapPinIcon, MenuIcon, CheckCircleIcon, ClockIcon,
// } from 'lucide-react';
...
//   createdAt: string;
//   status?: 'open' | 'pending' | 'fulfilled' | 'closed';
//   acceptedBy?: string;
//   acceptedAt?: string;
// }
...
//   isMe: boolean; 
//   userRole: string;
//   onAcceptDirectly: (req: BloodRequest) => void;
// }) {
...
//     switch (status) {
//       case 'pending':
//         return <Badge variant="warning" size="sm" className="flex items-center gap-1"><ClockIcon className="w-3 h-3" /> Pending</Badge>;
//       case 'fulfilled':
//         return <Badge variant="success" size="sm" className="flex items-center gap-1"><CheckCircleIcon className="w-3 h-3" /> Fulfilled</Badge>;
//       case 'closed':
...
//   const [confirmModal, setConfirmModal] = useState<{ show: boolean; request: BloodRequest | null }>({ show: false, request: null });
...
//     setConfirmModal({ show: true, request });
//   };
...
//     setProcessing(true);
//     try {
//       // Step 1: Accept the blood request
//       const acceptUrl = `${API}/api/blood-requests/${confirmModal.request._id}/accept`;
//       console.log('Calling accept endpoint:', acceptUrl);
...
//       await acceptRes.json(); // Consume the response
      
//       // Step 2: Find or create connection with the donor and send notification message
//       const donorId = typeof confirmModal.request.userId === 'object' 
//         ? (confirmModal.request.userId as any)._id 
//         : confirmModal.request.userId;

//       // Check connection status
//       const statusRes = await fetch(`${API}/api/connections/status/${donorId}`, {
...
//           const connRes = await fetch(`${API}/api/connections/request`, {
//             method: 'POST',
//             headers: { 
//               'Authorization': `Bearer ${token}`,
//               'Content-Type': 'application/json'
//             },
//             body: JSON.stringify({ toUserId: donorId })
//           });
//           if (connRes.ok) {
//             const connData = await connRes.json();
//             connectionId = connData.connection._id;
//           }
//         } else if (statusData.status === 'pending') {
//           // Connection request exists, use it
//           connectionId = statusData.connectionId;
//         }
//       }

//       // Step 3: Send notification message to donor with action marker
//       if (connectionId) {
//         const hospitalName = me?.hospitalName || me?.name || 'Hospital';
//         const bloodGroup = confirmModal.request.bloodGroup;
//         const requestId = confirmModal.request._id;
        
//         // Send a special formatted message that the frontend can detect and add buttons to
//         const notificationMessage = `🩸 BLOOD_REQUEST_NOTIFICATION 🩸\nHospital: ${hospitalName}\nBlood Group: ${bloodGroup}\nRequest ID: ${requestId}\n\n${hospitalName} has accepted your blood request for ${bloodGroup} blood. Please check your messages to respond.`;
        
//         await fetch(`${API}/api/connections/${connectionId}/messages`, {
//           method: 'POST',
//           headers: { 
//             'Authorization': `Bearer ${token}`,
//             'Content-Type': 'application/json'
...
//         req._id === confirmModal.request?._id 
//           ? { ...req, status: 'pending', acceptedBy: myId, acceptedAt: new Date().toISOString() }
//           : req
//       ));

//       showToast('Blood request accepted! Donor has been notified via message.', 'success');
//       setConfirmModal({ show: false, request: null });
...
//             {/* List View */}
//             <div className="space-y-6" style={{ display: viewMode === 'list' ? 'block' : 'none' }}>
...
//                       const uid = typeof req.userId === 'object' ? (req.userId as any)._id : req.userId;
//                       return <BloodRequestCard key={req._id} req={req} isMe={uid === myId} userRole={userRole} onAcceptDirectly={handleAcceptRequestDirectly} />;
//                     })}
//                   </div>
//                 </section>
//               )}
...
//                       const uid = typeof req.userId === 'object' ? (req.userId as any)._id : req.userId;
//                       return <BloodRequestCard key={req._id} req={req} isMe={uid === myId} userRole={userRole} onDonateClick={handleDonateClick} />;
//                     })}
//                   </div>
//                 </section>
//               )}
...
//         onClose={() => !processing && setConfirmModal({ show: false, request: null })}
...
//                 You are about to accept a blood request from:
//               </p>
//               <div className="space-y-2">
//                 <p className="font-semibold text-gray-900">{confirmModal.request.name}</p>
//                 <p className="text-sm text-gray-600">Blood Group: <span className="font-semibold text-primary">{confirmModal.request.bloodGroup}</span></p>
//                 <p className="text-sm text-gray-600">Phone: {confirmModal.request.phone}</p>
//                 <p className="text-sm text-gray-600">Email: {confirmModal.request.email}</p>
//                 {confirmModal.request.isEmergency && (
...
//                 <li>The donor will receive a notification about your acceptance</li>
//                 <li>The request status will be marked as "Pending"</li>
//                 <li>You can contact the donor to arrange the donation</li>
//                 <li>Once donation is complete, mark it as "Fulfilled"</li>
//               </ul>
//             </div>
...
//                 variant="outline"
//                 fullWidth
//                 onClick={() => setConfirmModal({ show: false, request: null })}
//                 disabled={processing}
//               >
//                 Cancel
//               </Button>
...
function BloodRequestCard({
  ...
  return (
    <div style={{ border: `2px solid ${border}`, borderRadius: 12, padding: 16, background: bg }}>
      ...
      {/* ✅ Fixed: show Accept button for hospital role on open requests that aren't theirs */}
      {!isMe && userRole === 'hospital' && status === 'open' && (
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(0,0,0,0.1)' }}>
          <Button
            fullWidth
...
export function BloodRequestsPage() {
  ...
  // ✅ Fixed: single handler that opens the confirmation modal (used for both emergency and normal cards)
  const handleDonateClick = (request: BloodRequest) => {
    setConfirmModal({ show: true, request });
  };
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      ...
      if (connectionId) {
        ...
        // Send a simple message with hospital name and request ID for parsing
        const notificationMessage = `🩸 BLOOD_REQUEST_NOTIFICATION 🩸\nHospital: ${hospitalName}\nRequest ID: ${requestId}\n\n${hospitalName} has accepted your blood request for ${bloodGroup} blood. Please check your messages to respond.`;
      ...
      setBloodRequests(prev => prev.map(req =>
        req._id === confirmModal.request?._id
          ? { ...req, status: 'fulfilled', acceptedBy: myId, acceptedAt: new Date().toISOString() }
          : req
      ));
                      ...
                      {emergencyRequests.map(req => {
                        ...
                        return (
                          <BloodRequestCard
                            ...
                            isMe={uid === myId}
                            userRole={userRole}
                            onAcceptDirectly={handleDonateClick} // ✅ Fixed: was handleAcceptRequestDirectly (undefined)
                          />
                        );
                      })}
                    </div>
                      ...
                      {normalRequests.map(req => {
                        ...
                        return (
                          <BloodRequestCard
                            ...
                            isMe={uid === myId}
                            userRole={userRole}
                            onAcceptDirectly={handleDonateClick} // ✅ Fixed: was wrong prop name onDonateClick
                          />
                        );
                      })}
                    </div>
