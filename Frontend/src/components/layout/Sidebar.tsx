...
import { Link, useLocation, useNavigate } from 'react-router-dom';
import {
  LayoutDashboardIcon, HistoryIcon, CalendarIcon, BellIcon,
  SettingsIcon, MessageCircleIcon, UsersIcon, BuildingIcon,
  FileTextIcon, ShieldCheckIcon, BarChart3Icon, HeartPulseIcon,
  XIcon, ChevronLeftIcon, LogOutIcon,
} from 'lucide-react';
...
const userNavItems: NavItem[] = [
  ...
  { href: '/dashboard/history', label: 'Donation History', icon: <HistoryIcon className="w-5 h-5" /> },
  { href: '/dashboard/camps', label: 'Blood Camps', icon: <CalendarIcon className="w-5 h-5" /> },
  { href: '/dashboard/requests', label: 'Blood Requests', icon: <BellIcon className="w-5 h-5" /> },
  { href: '/dashboard/chat', label: 'Messages', icon: <MessageCircleIcon className="w-5 h-5" /> },
  { href: '/dashboard/settings', label: 'Settings', icon: <SettingsIcon className="w-5 h-5" /> },
];

const hospitalNavItems: NavItem[] = [
  { href: '/hospital/dashboard', label: 'Dashboard', icon: <LayoutDashboardIcon className="w-5 h-5" /> },
  { href: '/hospital/requests', label: 'Blood Requests', icon: <HeartPulseIcon className="w-5 h-5" /> },
  { href: '/hospital/donors', label: 'Connected Donors', icon: <UsersIcon className="w-5 h-5" /> },
  { href: '/hospital/connection-requests', label: 'Connection Requests', icon: <BellIcon className="w-5 h-5" /> },
  { href: '/hospital/chat', label: 'Messages', icon: <MessageCircleIcon className="w-5 h-5" /> },
  { href: '/hospital/camps', label: 'Donation Camps', icon: <CalendarIcon className="w-5 h-5" /> },
  { href: '/hospital/records', label: 'Donation Records', icon: <FileTextIcon className="w-5 h-5" /> },
  { href: '/hospital/settings', label: 'Settings', icon: <SettingsIcon className="w-5 h-5" /> },
];

const adminNavItems: NavItem[] = [
  { href: '/admin/dashboard', label: 'Dashboard', icon: <LayoutDashboardIcon className="w-5 h-5" /> },
  { href: '/admin/camps', label: 'Camp Approvals', icon: <CalendarIcon className="w-5 h-5" /> },
  { href: '/admin/reports', label: 'Reports & Analytics', icon: <BarChart3Icon className="w-5 h-5" /> },
  { href: '/admin/settings', label: 'Settings', icon: <SettingsIcon className="w-5 h-5" /> },
];
...
/** Returns true only if a connection object has a valid non-null _id */
function isValidConnection(conn: any): boolean {
  return (
    conn !== null &&
    conn !== undefined &&
    typeof conn === 'object' &&
  ...
  useEffect(() => {
    ...
    const fetchBloodRequestsCount = async () => {
      try {
        ...
        const activeRequests = data.filter((req: any) => req && req.isActive === true);
        setBloodRequestsCount(activeRequests.length);
      } catch (err) {
        console.debug('Failed to fetch blood requests count:', err);
      }
  ...
  useEffect(() => {
    const fetchUnreadCount = async () => {
      try {
        ...
        // Guard: must be a non-empty array of valid connections
        if (!Array.isArray(connections) || connections.length === 0) return;

        // Filter out any null/invalid connections before mapping
        const validConnections = connections.filter(isValidConnection);
        if (validConnections.length === 0) return;

        const previewPromises = validConnections.map(async (conn: any) => {
          try {
            const r = await fetch(`${API}/api/connections/${conn._id}/preview`, {
              headers: {
                Authorization: `Bearer ${token}`,
            ...
            if (r.ok) {
              const data = await r.json();
              return (data && data.unreadCount > 0) ? 1 : 0;
            }
          } catch {}
          return 0;
        });
        ...
        const results = await Promise.all(previewPromises);
        const count = results.reduce((sum: number, val: number) => sum + val, 0);
        setUnreadMessageUsers(count);
      } catch (err) {
        console.debug('Failed to fetch unread count:', err);
      }
      ...
      >
        ...
        {onToggleCollapse && (
          <button
            ...
            className="hidden lg:flex absolute -right-3 bottom-20 w-6 h-6 bg-white border border-gray-200 rounded-full items-center justify-center hover:bg-gray-50 transition-colors shadow-sm"
