...
function ConvItem({ conn, myId, preview, onClick, active }: {
  ...
  // ✅ Add null checks for from and to objects
  if (!conn.from || !conn.to || !conn.from._id || !conn.to._id) {
    return null; // Skip rendering if connection data is invalid
  }
  ...
  const subtitle = (() => {
    if (!latest || !latest.sender || !latest.sender._id) {
      // No messages yet or invalid sender — fall back to blood group
  ...
  const fetchConnections = useCallback(async (): Promise<Connection[]> => {
    try {
      ...
      if (r.ok) {
        ...
        // ✅ Filter out connections with null from/to objects
        const validConnections = data.filter(conn => 
          conn.from && conn.to && conn.from._id && conn.to._id
        );
  ...
  useEffect(() => {
    const handler = async (e: Event) => {
      ...
      const conn  = conns.find(c => 
        (c.from && c.from._id === userId) || (c.to && c.to._id === userId)
      );
    ...
    const load = async () => {
      try {
        ...
        if (r.ok) {
          ...
          const validMessages = data.filter((msg: ChatMessage) => msg.sender && msg.sender._id);
                ...
                <div className="flex-1 overflow-y-auto bg-white">
                  {connections.length === 0 ? (
                    ...
                    <div className="p-2 space-y-1">
                      ...
                      {[...connections]
                        .filter(conn => conn && conn._id && conn.from && conn.to) // ✅ Filter out invalid connections
                        .sort((a, b) => {
...
.map(conn => {
  if (!conn || !conn._id) return null; // ← add this guard
  return (
    <ConvItem
      key={conn._id}
      conn={conn}
      myId={myId}
      preview={previews[conn._id]}
      active={Boolean(activeConn && activeConn._id === conn._id)}
                ...
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50">
                    {messagesLoading ? (
                      ...
                      messages
                        .filter(msg => msg.sender && msg.sender._id) // ✅ Filter out messages with null senders
                        .map((msg, index) => {
