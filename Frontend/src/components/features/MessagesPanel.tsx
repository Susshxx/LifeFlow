...
/** Only accept connections where both from and to are fully populated non-null objects */
function isValidConnection(conn: any): conn is Connection {
  return (
    ...
    conn.from._id.length > 0 &&
    typeof conn.from.name === 'string' &&
    conn.from.name.length > 0 &&
    conn.to !== null &&
    conn.to !== undefined &&
    typeof conn.to === 'object' &&
    typeof conn.to._id === 'string' &&
    conn.to._id.length > 0 &&
    typeof conn.to.name === 'string' &&
    conn.to.name.length > 0
  );
}

/** Only accept messages where sender is a fully populated non-null object */
function isValidMessage(msg: any): msg is ChatMessage {
  return (
    ...
    typeof msg.sender._id === 'string' &&
    msg.sender._id.length > 0 &&
...
export function MessagesPanel({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter out any null/invalid connections before doing anything else
        const validConns: Connection[] = rawConns.filter(isValidConnection);
        ...
        // Fetch previews only for valid connections
        const previewResults = await Promise.all(
          validConns.map(async (c) => {
            try {
              const r = await fetch(`${API}/api/connections/${c._id}/preview`, {
                headers: authHeaders(),
              });
              if (r.ok) {
                const data = await r.json();
                // Validate the latest message inside preview too
                if (data && data.latest && !isValidMessage(data.latest)) {
                  data.latest = null;
                }
                return { id: c._id, preview: data as ConvPreview };
              }
  ...
  // Sort valid connections by latest message timestamp (most recent first)
  const sortedConnections = [...connections].sort((a, b) => {
    const previewA = previews[a._id];
    const previewB = previews[b._id];
    const timeA = previewA?.latest?.createdAt
      ? new Date(previewA.latest.createdAt).getTime()
      : 0;
    const timeB = previewB?.latest?.createdAt
      ? new Date(previewB.latest.createdAt).getTime()
      : 0;
    return timeB - timeA;
  });
    ...
    >
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <div className="divide-y divide-gray-100">
            {sortedConnections.map((conn) => {
              // Extra in-render guard — belt and suspenders
              if (!isValidConnection(conn)) return null;

              // Safe: both from and to are guaranteed non-null here
              const other = conn.from._id === myId ? conn.to : conn.from;
              const preview = previews[conn._id];
              const unread = preview?.unreadCount || 0;
              const latest = preview?.latest && isValidMessage(preview.latest)
                ? preview.latest
                : null;
